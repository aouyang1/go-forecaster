...
func testFitConstantSignal(t *testing.T) (*Forecast, []time.Time, []float64) {
	// create a daily sine wave at minutely with one week
	minutes := 7 * 24 * 60
	bias := 7.9
...
func TestFit(t *testing.T) {
	testData := map[string]struct {
		sigFunc  func(*testing.T) (*Forecast, []time.Time, []float64)
		expected []float64
	}{
		"constant": {
			sigFunc:  testFitConstantSignal,
			expected: []float64{7.9},
		},
		"seasonal": {
			sigFunc:  testFitSignal,
			expected: []float64{7.90, 3.72, 2.14, 0.00, 0.00, 0.00, 0.00},
		},
	}
	...
	for name, td := range testData {
		t.Run(name, func(t *testing.T) {
			...
			assert.InDeltaSlice(t, td.expected, res, 0.1)
