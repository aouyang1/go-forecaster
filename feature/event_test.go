...
func TestEventGenerate(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			times:       []time.Time{baseTime, baseTime.Add(24 * time.Hour), baseTime.Add(48 * time.Hour)},
			window:      WindowRectangular,
			evSetting:   &EventStandard{Start: baseTime.Add(12 * time.Hour), End: baseTime.Add(36 * time.Hour)},
			wantLen:     3,
			wantErr:     false,
		...
		},
		{
			...
			window:      WindowRectangular,
			evSetting:   &EventWeekend{DurBefore: 0, DurAfter: 0},
			wantLen:     2,
			wantErr:     false,
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			if weekend, ok := tc.evSetting.(*EventWeekend); ok {
				weekend.Validate()
			}

			got, err := tc.event.Generate(tc.times, tc.window, tc.evSetting)
